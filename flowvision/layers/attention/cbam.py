...
class ChannelAttn(nn.Module):
    ...
    def __init__(
            ...
            act_layer=nn.ReLU, gate_layer=nn.Sigmoid, mlp_bias=False):
    ...
    def forward(self, x):
        ...
        x_max = self.fc2(self.act(self.fc1(F.max_pool2d(x, kernel_size=(x.size(2), x.size(3)), stride=(x.size(2), x.size(3))))))
...
class SpatialAttn(nn.Module):
    ...
    def __init__(self, kernel_size=7, gate_layer=nn.Sigmoid):
        ...
        self.conv = ConvBnAct(2, 1, kernel_size=kernel_size, padding=kernel_size//2, act_layer=None)
        self.gate = gate_layer()

    def forward(self, x):
        # TODO: switch flow.max to tensor.amax
        x_attn = flow.cat([x.mean(dim=1, keepdim=True), flow.max(x, dim=1, keepdim=True)[0]], dim=1)
...
class CbamModule(nn.Module):
    def __init__(
            self, channels, rd_ratio=1./16, rd_channels=None, rd_divisor=1,
            spatial_kernel_size=7, act_layer=nn.ReLU, gate_layer=nn.Sigmoid, mlp_bias=False):
