...
class CoordAttModule(nn.Module):
    def __init__(
            ...
            act_layer=hard_swish, gate_layer=nn.Sigmoid, mlp_bias=False):
        super(CoordAttModule, self).__init__()
        if not rd_channels:
            rd_channels = make_divisible(channels * rd_ratio, rd_divisor, min_value=8, round_limit=0.)
        out_channels = channels if out_channels is None else out_channels

        self.pool_h = nn.AdaptiveAvgPool2d((None, 1))
        self.pool_w = nn.AdaptiveAvgPool2d((1, None))

        self.conv1 = nn.Conv2d(channels, rd_channels, kernel_size=1, stride=1, padding=0, bias=mlp_bias)
        self.bn1 = nn.BatchNorm2d(rd_channels)
        self.act = act_layer()

        self.conv_h = nn.Conv2d(rd_channels, out_channels, kernel_size=1, stride=1, padding=0, bias=mlp_bias)
        self.conv_w = nn.Conv2d(rd_channels, out_channels, kernel_size=1, stride=1, padding=0, bias=mlp_bias)
