...
from flowvision.layers import DropPath, Mlp, trunc_normal_ as __call_trunc_normal_
...
class Attention(nn.Module):
    def __init__(
        ...
        if window_size:
            ...
            coords_h = flow.arange(window_size[0])
            coords_w = flow.arange(window_size[1])
            ...
            relative_position_index = \
                flow.zeros(size=(window_size[0] * window_size[1] + 1,) * 2, dtype=relative_coords.dtype)
    ...
    def forward(self, x, rel_pos_bias=None):
        ...
        if self.q_bias is not None:
            qkv_bias = flow.cat((self.q_bias, flow.zeros_like(self.v_bias, requires_grad=False), self.v_bias))
...
class VisionTransformerForMaskedImageModeling(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_chans=3, vocab_size=8192, embed_dim=768, depth=12,
        ...
        if use_abs_pos_emb:
            self.pos_embed = nn.Parameter(flow.zeros(1, num_patches + 1, embed_dim))
        else:
            self.pos_embed = None
