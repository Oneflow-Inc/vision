...
def gen_relative_position_index(window_size: Tuple[int, int]) -> flow.Tensor:
    ...
    coords = flow.stack(flow.meshgrid(
        [flow.arange(window_size[0]),
         flow.arange(window_size[1])]))  # 2, Wh, Ww
    ...
    relative_position_index = flow.zeros((window_area + 1,) * 2, dtype=relative_coords.dtype)
...
class Attention(nn.Module):
    def __init__(
        ...
        if window_size:
            ...
            self.relative_position_bias_table = nn.Parameter(
                flow.zeros(self.num_relative_distance, num_heads))  # 2*Wh-1 * 2*Ww-1, nH
    ...
    def forward(self, x, shared_rel_pos_bias: Optional[flow.Tensor] = None):
        ...
        qkv_bias = flow.cat((self.q_bias, self.k_bias, self.v_bias)) if self.q_bias is not None else None
...
class RelativePositionBias(nn.Module):
    ...
    def __init__(self, window_size, num_heads):
        ...
        num_relative_distance = (2 * window_size[0] - 1) * (2 * window_size[1] - 1) + 3
        self.relative_position_bias_table = nn.Parameter(flow.zeros(num_relative_distance, num_heads))
...
class Beit(nn.Module):
    ...
    def __init__(
        ...
        self.pos_embed = nn.Parameter(flow.zeros(1, num_patches + 1, embed_dim)) if use_abs_pos_emb else None
