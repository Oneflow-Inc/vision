...
class PositionalEncodingFourier(nn.Module):
    ...
    def forward(self, B, H, W):
        mask = flow.zeros((B, H, W),dtype=flow.bool).to(self.token_projection.weight.device)
        not_mask = ~mask
        ...
        dim_t = flow.arange(self.hidden_dim, dtype=flow.float32, device=mask.device)
...
class LPI(nn.Module):
    ...
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU,
        ...
        self.conv1 = flow.nn.Conv2d(in_features, out_features, kernel_size=kernel_size,
                                     padding=padding, groups=out_features)
        self.act = act_layer()
        self.bn = nn.BatchNorm2d(in_features)
        self.conv2 = flow.nn.Conv2d(in_features, out_features, kernel_size=kernel_size,
                                     padding=padding, groups=out_features)
...
class ClassAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        x = flow.cat([self.proj_drop(cls_tkn), x[:, 1:]], dim=1)
        return x
