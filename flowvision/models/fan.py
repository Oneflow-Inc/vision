...
class ConvNeXtBlock(nn.Module):
    ...
    def __init__(self, dim, drop_path=0., ls_init_value=1e-6, conv_mlp=True, mlp_ratio=4, norm_layer=None):
        ...
        self.gamma = nn.Parameter(ls_init_value * flow.ones(dim)) if ls_init_value > 0 else None
...
class ConvNeXt(nn.Module):
    ...
    def __init__(
        ...
        dp_rates = [x.tolist() for x in flow.linspace(0, drop_path_rate, sum(depths)).split(depths)]
    ...
    def __init__(self, backbone, img_size=224, patch_size=2, feature_size=None, in_chans=3, embed_dim=384):
        ...
        if feature_size is None:
            with flow.no_grad():
                ...
                if training:
                    backbone.eval()
                o = self.backbone.forward_features(flow.zeros(1, in_chans, img_size[0], img_size[1]))
