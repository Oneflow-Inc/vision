...
class AnchorGenerator(nn.Module):
    """
    Module that generates anchors for a set if feature maps and
    image sizes.
    ...
    def generate_anchors(
        ...
        scales = flow.as_tensor(scales, dtype=dtype, device=device)
        aspect_ratios = flow.as_tensor(aspect_ratios, dtype=dtype, device=device)
    ...
    def set_cell_anchors(self, dtype: flow.dtype, device: flow.device):
        self.cell_anchors = [
    ...
    def grid_anchors(
        ...
        for size, stride, base_anchors in zip(grid_sizes, strides, cell_anchors):
            ...
            shifts_x = (
                flow.arange(0, grid_width, dtype=flow.int32, device=device)
                * stride_width
            )
            shifts_y = (
                flow.arange(0, grid_height, dtype=flow.int32, device=device)
                * stride_height
            )
    ...
    def forward(
        ...
        strides = [
            [
                flow.tensor(image_size[0] // g[0], dtype=flow.int64, device=device),
                flow.tensor(image_size[1] // g[1], dtype=flow.int64, device=device),
            ]
            for g in grid_sizes
        ]
        self.set_cell_anchors(dtype, device)
        anchors_over_all_feature_maps = self.grid_anchors(grid_sizes, strides)
        anchors: List[List[flow.Tensor]] = []
        ...
        anchors = [flow.cat(anchors_per_image) for anchors_per_image in anchors]
        return anchors
...
class DefaultBoxGenerator(nn.Module):
    ...
    def _grid_default_boxes(
        ...
        for k, f_k in enumerate(grid_sizes):
            ...
            shifts_x = ((flow.arange(0, f_k[1], device=flow.device("cpu")) + 0.5) / x_f_k).to(dtype=dtype)
            shifts_y = ((flow.arange(0, f_k[0], device=flow.device("cpu")) + 0.5) / y_f_k).to(dtype=dtype)
            shift_y, shift_x = flow.meshgrid(shifts_y, shifts_x)
            shift_x = shift_x.reshape(-1)
            shift_y = shift_y.reshape(-1)
