        ...
        if self.training:
            ...
            for anchors_per_image, targets_per_image in zip(anchors, targets):
                if targets_per_image['boxes'].numel() == 0:
                    matched_idxs.append(flow.full((anchors_per_image.size(0),), -1, dtype=flow.int64,
    ...
    def postprocess_detections(self, head_outputs: Dict[str, Tensor], image_anchors: List[Tensor],
        ...
        for boxes, scores, anchors, image_shape in zip(bbox_regression, pred_scores, image_anchors, image_shapes):
            ...
            for label in range(1, num_classes):
                ...
                image_labels.append(flow.full(score.shape, value=label, dtype=flow.int64, device=device))
...
def ssd300_vgg16(pretrained: bool = False, progress: bool = True, num_classes: int = 91,
                 pretrained_backbone: bool = False, trainable_backbone_layers: Optional[int] = None, **kwargs: Any):
    ...
    backbone = _vgg_extractor("vgg16", False, progress, pretrained_backbone, trainable_backbone_layers)
    ...
    if pretrained:
        ...
        if model_urls.get(weights_name, None) is None:
            raise ValueError("No checkpoint is availabel for model {}".format(weights_name))
