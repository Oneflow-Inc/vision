...
from flowvision.layers import sigmoid_focal_loss, generalized_box_iou_loss
...
class FCOSHead(nn.Module):
    ...
    def compute_loss(
        ...
        all_gt_classes_targets = flow.stack(all_gt_classes_targets)
        # compute foreground
        foreground_mask = all_gt_classes_targets >= 0
        num_foreground = foreground_mask.sum().item()

        # classification loss
        gt_classes_targets = flow.zeros_like(cls_logits)
        gt_classes_targets[
            foreground_mask, all_gt_classes_targets[foreground_mask]
        ] = 1.0
        ...
        pred_boxes = [
            ...
            for anchors_per_image, bbox_regression_per_image in zip(
                anchors, bbox_regression
            )
        ]
        loss_bbox_reg = generalized_box_iou_loss(
            flow.stack(pred_boxes)[foreground_mask],
            flow.stack(all_gt_boxes_targets)[foreground_mask],
            reduction="sum",
        )
        ...
        bbox_reg_targets = [
            ...
            for anchors_per_image, boxes_targets_per_image in zip(
                anchors, all_gt_boxes_targets
            )
        ]
        bbox_reg_targets = flow.stack(bbox_reg_targets, dim=0)
        if len(bbox_reg_targets) == 0:
            ...
            gt_ctrness_targets = flow.sqrt(
                (left_right.min(dim=-1)[0] / left_right.max(dim=-1)[0])
                * (top_bottom.min(dim=-1)[0] / top_bottom.max(dim=-1)[0])
            )
        pred_centerness = bbox_ctrness.squeeze(dim=2)
        loss_bbox_ctrness = flow._C.binary_cross_entropy_with_logits_loss(
            pred_centerness[foreground_mask],
            gt_ctrness_targets[foreground_mask],
            reduction="sum",
        )
...
class FCOSRegressionHead(nn.Module):
    ...
    def forward(self, x: List[Tensor]) -> Tuple[Tensor, Tensor]:
        ...
        return flow.cat(all_bbox_regression, dim=1), flow.cat(all_bbox_ctrness, dim=1)
...
class FCOS(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if anchor_generator is None:
            ...
            aspect_ratios = ((1.0,),) * len(anchor_sizes)  # set only on anchor
    ...
    def compute_loss(
        ...
        for anchors_per_image, targets_per_image in zip(anchors, targets):
            ...
            gt_centers = (gt_boxes[:, :2] + gt_boxes[:, 2:]) / 2  # Mx2
            anchor_centers = (
                anchors_per_image[:, :2] + anchors_per_image[:, 2:]
            ) / 2  # Nx2
            ...
            ).abs().max(dim=2)[0] < self.center_sampling_radius * anchor_sizes[:, None]
            # compute pairwise distance between N points and M boxes
            x, y = anchor_centers.unsqueeze(dim=2).unbind(dim=1)  # (N, 1)
            x0, y0, x1, y1 = gt_boxes.unsqueeze(dim=0).unbind(dim=2)  # (1, M)
            pairwise_dist = flow.stack(
                [x - x0, y - y0, x1 - x, y1 - y], dim=2
            )  # (N, M, 4)

            # anchor point must be inside gt
            pairwise_match &= pairwise_dist.min(dim=2)[0] > 0
            ...
            pairwise_dist = pairwise_dist.max(dim=2)[0]
            ...
            pairwise_match = pairwise_match.to(flow.float32) * (1e8 - gt_areas[None, :])
            min_values, matched_idx = pairwise_match.max(dim=1)  # N, per-anchor match
    ...
    def postprocess_detections(
        ...
        for index in range(num_images):
            ...
            for (
                ...
                logits_per_image,
                box_ctrness_per_image,
                anchors_per_image,
            ):
                num_classes = logits_per_level.shape[-1]

                # remove low scoring boxes
                scores_per_level = flow.sqrt(
                    flow.sigmoid(logits_per_level) * flow.sigmoid(box_ctrness_per_level)
                ).flatten()
        ...
        if self.training:
            if targets is None:
                ...
                losses = self.compute_loss(
                    targets, head_outputs, anchors, num_anchors_per_level
                )
        else:
...
def fcos_resnet50_fpn(
    ...
    **kwargs,
    ...
    During inference, the model requries only the input tensors, and returns the post-processed
    predictions as a ``List[Dict[Tensor]]``, one for each input image. The fields of the ``Dict`` are as
    ...
    .. code-block:: python
        ...
        >>> import flowvision
        >>> fcos_resnet50_fpn = flowvision.models.detection.fcos_resnet50_fpn(pretrained=False, progress=True)
    
    """
    weights_name = "fcos_resnet50_fpn_coco"
    backbone_name = "resnet50"
    return _fcos_resnet_fpn(
        weights_name,
        backbone_name,
