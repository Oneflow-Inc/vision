...
class SABlock(nn.Module):
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        if self.ls:
            ...
            self.gamma_1 = nn.Parameter(init_value * flow.ones((dim)),requires_grad=True)
            self.gamma_2 = nn.Parameter(init_value * flow.ones((dim)),requires_grad=True)
...
def uniformer_small(pretrained=False, progress=True, **kwargs):
    ...
    return _create_uniformer("uniformer_small", pretrained=pretrained, progress=progress, **model_kwargs)
...
def uniformer_base(pretrained=False, progress=True, **kwargs):
    ...
    model_kwargs = dict(
        ...
        embed_dim=[64, 128, 320, 512], head_dim=64, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return _create_uniformer("uniformer_base", pretrained=pretrained, progress=progress, **model_kwargs)
...
def uniformer_base_ls(pretrained=False, progress=True, **kwargs):
    ...
    model_kwargs = dict(
        ...
        embed_dim=[64, 128, 320, 512], head_dim=64, mlp_ratio=4, qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), **kwargs)
    return _create_uniformer("uniformer_base_ls", pretrained=pretrained, progress=progress, **model_kwargs)
