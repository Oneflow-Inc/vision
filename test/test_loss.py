...
from flowvision.loss.cross_entropy import SoftTargetCrossEntropy, LabelSmoothingCrossEntropy
...
def _test_label_smooth(test_case, shape, device, smoothing):
    ...
    of_predict = flow.tensor(
        np_predict, dtype=flow.float32, device=flow.device(device)
    )
    of_label = flow.tensor(
        np_label, dtype=flow.int64, device=flow.device(device)
    )
    ...
    test_case.assertTrue(np.allclose(np_smooth_loss, of_smooth_loss.numpy(), 1e-4, 1e-4, equal_nan=True))
