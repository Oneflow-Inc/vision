...
def _coco_remove_images_without_annotations(dataset, cat_list=None):
    ...
    assert isinstance(dataset, flowvision.datasets.CocoDetection)
    ids = []
...
class CocoDetection(flowvision.datasets.CocoDetection):
    def __init__(self, img_foler, ann_file, transforms):
        super(CocoDetection, self).__init__(img_foler, ann_file)
...
def get_coco(root, image_set, transforms, mode="instances"):
    ...
    img_foler, ann_file = PATHS[image_set]
    img_foler = os.path.join(root, img_foler)
    ann_file = os.path.join(root, ann_file)

    dataset = CocoDetection(img_foler, ann_file, transforms=transforms)
